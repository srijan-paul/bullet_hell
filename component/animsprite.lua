...
function AnimatedSprite:play(key, callback)
    if self.anims[key] then
        ...
        if callback then self.current.callback = callback end
    end
end
...
function AnimatedSprite:is_playing(key)
    if self.current then
        if key then
            return self.anims[key] == self.current and self.current.playing
        end
...
function AnimatedSprite:draw()
    ...
    -- offset
    local dp = Vec2(self.frame_width / 2, self.frame_height / 2):rotated(
                   t.rotation)
    local x = t.pos.x - dp.x
    local y = t.pos.y - dp.y
    if t.scale.x < 0 then x = x + self.frame_width * -1 * t.scale.x end

    self.current:draw(x, y, t.rotation, t.scale.x, t.scale.y)
end

function AnimatedSprite:update(dt) self.current:update(dt) end

function AnimatedSprite:delete() self.owner.world:remove_drawable(self) end
